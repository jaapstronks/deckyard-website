...
Dezelfde spanning is in deze repo al een keer beslecht, en dat is het voorbeeld dat je volgt: de site publiceert alleen schema's die op een core-release-tag staan, niet wat core's `HEAD` toevallig zegt (zie `src/data/CLAUDE.md`, en de sessie achter `b4e75ea`).
